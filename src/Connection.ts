...
class Connection extends EventEmitter {
  ...
  writeAudio(packet: any, whisperTarget: any, codec: any, voiceSequence: any, final: any) {
    ...
    if (codec == Connection.codec().Opus) {
      if (packet.length > 0x1fff) throw new TypeError(`Audio frame too long! Max Opus length is ${0x1fff} bytes.`)
